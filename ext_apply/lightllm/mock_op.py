...
import os
try:
    #export DISABLE_DIOPI_OP_LIST=dest_index_copy_kv,token_attention_inference,token_softmax_reducev_inference
    import dipu_ext.ext_ as ext
    disable_diopi_op_list = os.environ['DISABLE_DIOPI_OP_LIST'].split(',')
    print(f"DISABLE_DIOPI_OP_LIST:{disable_diopi_op_list}")
    if hasattr(ext, 'dest_index_copy_kv') and 'dest_index_copy_kv' not in disable_diopi_op_list:
    ...
    if hasattr(ext, 'apply_penalty') and 'apply_penalty' not in disable_diopi_op_list:
    ...
    if hasattr(ext, 'context_attention_inference') and 'context_attention_inference' not in disable_diopi_op_list:
    ...
    if hasattr(ext, 'token_attention_inference') and 'token_attention_inference' not in disable_diopi_op_list:
    ...
    if hasattr(ext, 'token_softmax_reducev_inference') and 'token_softmax_reducev_inference' not in disable_diopi_op_list:
