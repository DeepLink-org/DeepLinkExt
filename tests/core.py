...
def allclose(expected_vals:list, real_vals:list, rtol, atol):
    ...
    for i in range(len(expected_vals)):
        assert type(expected_vals[i]) == type(real_vals[i]), "the type of expected_vals[{index}] is {type1}, but real_vals[{index}] is {type2}.".format(index=i, type1=type(expected_vals[i]), type2=type(real_vals[i]))
        if isinstance(expected_vals[i], torch.Tensor):
            assert isinstance(real_vals[i], torch.Tensor)
            return torch.allclose(expected_vals[i].cpu().to(torch.float32), real_vals[i].cpu().to(torch.float32), rtol, atol)
